...
# from nsvqa.nn.interpreter.batch_attention_transfer import AttentionTransferModulator
from nsvqa.nn.vision.boxfeatures_oracle import BoxVisualOracle
from nsvqa.nn.vision.classifier_oracle import ClassifierOracle
from nsvqa.nn.vision.nilp_5 import VisualRuleLearner
from nsvqa.nn.vision.calibrator import Calibrator
...
class GQAObjectBoxExperiment(ExperimentBase):
    ...
    def build_interpreter(self, config, neural_dict, ontology, logger):
        ...
        # Build the oracle
        if config['oracle_output_dim'] == 1 and config['classifier_oracle']:
            if config['verbose'] and self._local_rank == 0:
            ...
            if config['verbose'] and self._local_rank == 0:
                logger.info('Building the Box Oracle...')
            oracle = BoxVisualOracle(ontology, neural_dict['attribute_network'], neural_dict['relation_network'], output_dim=config['oracle_output_dim'])
        ...
        if config['oracle_output_dim'] == 1:
            ...
            if 'calibrate_oracle' in config and config['calibrate_oracle']:
                if config['verbose'] and self._local_rank == 0:
                    logger.info('Building the oracle calibrator...')
                calibrator = Calibrator(len(ontology._attribute_index), len(ontology._relation_index), init_std=config['calibrator_std'])
            else:
                calibrator = None

            if 'learn_meta_rules' in config and config['learn_meta_rules']:
                if config['verbose'] and self._local_rank == 0:
                    logger.info('Building the visual rule learner...')
                visual_rule_learner = VisualRuleLearner(len(ontology._attribute_index), len(ontology._relation_index), config['rule_learner_hidden_dim'], \
                    config['head_count'], attribute_template_list=config['attribute_rule_template'], relation_template_list=config['relation_rule_template'], \
                    skip_connection=config['skip_connection'], init_std=config['init_std'], is_explicit=config['is_explicit'], layers_config=config['implicit_layers_config'], \
    ...
    def build_optimizer(self, trainer, config, logger):
        ...
        if 'apex' in config and config['apex']:
            if config['verbose'] and self._local_rank == 0:
                logger.info('Building the Fused Adam optimizer...')

            from apex.optimizers import FusedAdam
            return FusedAdam(trainer.get_parameter_list(), lr=config['learning_rate'], bias_correction=False, weight_decay=config['weight_decay'])
        else:     
            if config['verbose'] and self._local_rank == 0:
